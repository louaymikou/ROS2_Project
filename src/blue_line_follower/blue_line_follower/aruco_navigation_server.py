        ...
        try:
            ...
            if ids is not None and len(ids) > 0:
                ...
                if detected_id == self.current_aruco_id:
                    ...
                    self.aruco_detection_count = 1
                    # Log immédiat quand un nouveau marqueur est détecté
                    self.get_logger().info(f'🔍 NOUVEAU ArUco détecté: {detected_id}')
                
                if self.aruco_detection_count >= self.required_detections:
                    self.get_logger().info(
                        f'📍 ArUco {self.current_aruco_id} CONFIRMÉ (count: {self.aruco_detection_count})',
                        throttle_duration_sec=0.5
                    )
            else:
                # Pas de marqueur détecté - NE PAS décrémenter trop vite
                # Garder le dernier ArUco détecté pendant un moment
                cv2.putText(display_image, f'{camera_name} Camera - NO ArUco', 
                           (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 
                           1, (0, 0, 255), 2)
                
                # Décrémenter lentement (seulement si count > 0)
                # Ne pas réinitialiser current_aruco_id immédiatement
                if self.aruco_detection_count > 0:
                    self.aruco_detection_count = max(0, self.aruco_detection_count - 1)
    ...
    def execute_callback(self, goal_handle):
        ...
        while self.current_state == NavigationState.NAVIGATE_TO_TARGET:
            ...
            if current_id > 0 and current_id != last_detected_aruco:
                ...
                self.get_logger().info(f'🔄 ArUco {current_id} détecté (count: {self.aruco_detection_count}, target: {self.target_id})')
            
            # Log périodique pour debug
            self.get_logger().debug(f'ArUco check: current={self.current_aruco_id}, target={self.target_id}, count={self.aruco_detection_count}')
            ...
            # Vérifier si cible atteinte - condition plus souple
            if self.current_aruco_id == self.target_id:
                self.get_logger().info(f'🎯 ArUco {self.target_id} visible! (count: {self.aruco_detection_count}/{self.required_detections})', throttle_duration_sec=0.5)
                
                # Attendre d'avoir assez de détections pour confirmer
                if self.aruco_detection_count >= self.required_detections:
                    self.get_logger().info(f'🎯🎯 Cible {self.target_id} ATTEINTE ET CONFIRMÉE!')
                    time.sleep(0.5)
